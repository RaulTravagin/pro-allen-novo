    ...
    const data = buildGestorVisualProgress([
      {
        ...
        route: {
          ...
          totalPosts: 5,
          completedVisits: 2,
          visits: [{ status: "visited", occurrenceReport: "Visita concluída." }, { status: "visited" }, { status: "in_progress" }],
        },
      },
      { supervisorId: 2, supervisorName: "Rodrigo", operationalStatus: "sem_rota", alerts: [], route: null },
    ]);

    expect(data.supervisorProgress[0]).toMatchObject({ completed: 2, inProgress: 1, pending: 2, progress: 40, alertCount: 1, occurrenceTotal: 3, occurrenceReported: 1, occurrencePending: 2 });
