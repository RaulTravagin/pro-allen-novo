...
import { AlertTriangle, CheckCircle2, Download, HardDrive, LogOut, MapPin, Play, ShieldCheck, Upload } from "lucide-react";
...
export default function LocalContingency() {
  ...
  function updateVisit(visitId: string, operation: "checkin" | "checkout") {
    ...
    try {
      ...
      setActivity(next);
      refresh();
      toast.success(operation === "checkin" ? "Chegada registrada localmente." : "Ocorrência enviada e saída registrada localmente.");
    } catch (error) {
      toast.error(error instanceof Error ? error.message : "Não foi possível atualizar a visita.");
    }
  }

  function updateOccurrence(visitId: string, occurrenceReport: string) {
    if (!activity) return;
    const next = saveLocalVisitOccurrence(activity.id, visitId, occurrenceReport);
    setActivity(next);
    refresh();
  }
  ...
  return <div className="min-h-screen bg-slate-50 pb-10 text-slate-950"><header className="border-b border-slate-200 bg-slate-950 text-white"><div className="mx-auto flex max-w-6xl flex-wrap items-center justify-between gap-3 px-4 py-4"><div className="flex items-center gap-3"><div className="flex h-10 w-10 items-center justify-center rounded-xl bg-amber-400 text-slate-950"><HardDrive className="h-5 w-5" /></div><div><p className="text-xs font-bold uppercase tracking-[0.2em] text-amber-300">Pro Allen</p><h1 className="font-bold">Modo local de contingência</h1></div></div><div className="flex items-center gap-2"><Badge className="border border-amber-300/30 bg-amber-300/10 text-amber-200">{session.name}</Badge><Button size="sm" variant="outline" className="border-white/20 bg-transparent text-white hover:bg-white/10 hover:text-white" onClick={leaveLocalMode}><LogOut className="mr-2 h-4 w-4" />Sair</Button></div></div></header>
    <main className="mx-auto max-w-6xl space-y-5 px-4 py-6"><div className="flex gap-3 rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-950"><AlertTriangle className="mt-0.5 h-5 w-5 shrink-0" /><div><p className="font-semibold">Dados locais e não compartilhados.</p><p className="mt-1">Ocorrências, horários e KM registrados aqui não aparecem em outros celulares nem no Painel do Gestor até a recuperação manual por exportação e importação.</p></div></div>
      <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_320px]"><div className="space-y-5">{activity ? <LocalActivityPanel activity={activity} kmValue={kmValue} setKmValue={setKmValue} onSaveKm={saveKm} onVisitAction={updateVisit} onOccurrenceChange={updateOccurrence} /> : <StartActivityPanel routeId={routeId} setRouteId={setRouteId} onStart={startActivity} hasCompletedBase={completedActivities.some((item) => item.activityType === "operational_base")} />}</div>
...
function LocalActivityPanel({ activity, kmValue, setKmValue, onSaveKm, onVisitAction, onOccurrenceChange }: { activity: LocalActivity; kmValue: string; setKmValue: (value: string) => void; onSaveKm: (type: "initial" | "final") => void; onVisitAction: (visitId: string, action: "checkin" | "checkout") => void; onOccurrenceChange: (visitId: string, occurrenceReport: string) => void }) {
  ...
  return <div className="space-y-5"><Card className="border-slate-900 bg-slate-950 text-white"><CardHeader><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="text-xs font-bold uppercase tracking-[0.2em] text-amber-300">Atividade em andamento</p><CardTitle className="mt-1 text-2xl text-white">{activity.routeName}</CardTitle><CardDescription className="text-slate-300">{activity.routeRegion} · iniciado em {formatDateTime(activity.startedAt)}</CardDescription></div><Badge className="bg-emerald-400 text-emerald-950">Em andamento</Badge></div></CardHeader><CardContent><div className="grid gap-3 sm:grid-cols-2"><div className="rounded-lg border border-white/10 bg-white/5 p-3"><p className="text-xs text-slate-300">KM inicial</p><p className="mt-1 text-lg font-bold">{activity.kmInitial ?? "Pendente"}</p></div><div className="rounded-lg border border-white/10 bg-white/5 p-3"><p className="text-xs text-slate-300">KM final</p><p className="mt-1 text-lg font-bold">{activity.kmFinal ?? "Pendente"}</p></div></div><div className="mt-4 flex flex-col gap-2 sm:flex-row"><Input inputMode="decimal" value={kmValue} onChange={(event) => setKmValue(event.target.value)} placeholder={hasInitialKm ? "Informe o KM final" : "Informe o KM inicial"} className="border-white/20 bg-white text-slate-950" /><Button className="bg-amber-400 text-slate-950 hover:bg-amber-300" onClick={() => onSaveKm(hasInitialKm ? "final" : "initial")}>{hasInitialKm ? "Encerrar atividade" : "Registrar KM inicial"}</Button></div></CardContent></Card>
    {activity.activityType === "operational_base" ? <Card><CardContent className="flex gap-3 p-5"><ShieldCheck className="h-6 w-6 text-violet-700" /><div><p className="font-semibold">Atividade na Base Operacional</p><p className="mt-1 text-sm text-slate-600">Não há postos fictícios para a Base. Registre o KM inicial e, ao sair, informe o KM final para encerrar e abrir a seleção da próxima rota.</p></div></CardContent></Card> : <div className="space-y-4"><div><h2 className="text-xl font-bold">Postos e ocorrências locais</h2><p className="mt-1 text-sm text-slate-600">Cada visita precisa de um relato obrigatório antes da saída. Os dados são gravados neste navegador.</p></div>{activity.visits.map((visit) => <LocalVisitCard key={visit.id} visit={visit} canCheckIn={hasInitialKm} onAction={(action) => onVisitAction(visit.id, action)} onOccurrenceChange={(report) => onOccurrenceChange(visit.id, report)} />)}</div>}
  </div>;
}

function LocalVisitCard({ visit, canCheckIn, onAction, onOccurrenceChange }: { visit: LocalVisit; canCheckIn: boolean; onAction: (action: "checkin" | "checkout") => void; onOccurrenceChange: (report: string) => void }) {
  const occurrenceReady = visit.occurrenceReport.trim().length >= 8;
  return <Card className={visit.status === "in_progress" ? "border-emerald-300" : "border-slate-200"}><CardContent className="p-5"><div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div><div className="flex flex-wrap items-center gap-2"><h3 className="font-bold text-slate-950">{visit.postName}</h3><Badge className={visit.status === "in_progress" ? "bg-emerald-100 text-emerald-800" : visit.status === "visited" ? "bg-slate-100 text-slate-700" : "bg-amber-100 text-amber-800"}>{visit.status === "in_progress" ? "Em atendimento" : visit.status === "visited" ? "Visita concluída" : "Aguardando chegada"}</Badge></div><p className="mt-1 text-sm text-slate-600">{visit.region}</p>{visit.arrivalAt && <p className="mt-2 text-xs text-slate-600">Chegada: {formatDateTime(visit.arrivalAt)} · Permanência: {duration(visit.arrivalAt, visit.departureAt)}</p>}</div>{visit.status === "pending" ? <Button disabled={!canCheckIn} className="bg-emerald-600 hover:bg-emerald-700" onClick={() => onAction("checkin")}><MapPin className="mr-2 h-4 w-4" />Registrar chegada</Button> : visit.status === "in_progress" ? <Button disabled={!occurrenceReady} className="bg-amber-500 text-slate-950 hover:bg-amber-400" onClick={() => onAction("checkout")}><CheckCircle2 className="mr-2 h-4 w-4" />Enviar ocorrência e registrar saída</Button> : <Badge className="bg-slate-100 text-slate-700">Concluída</Badge>}</div>{visit.status !== "pending" && <div className="mt-4 space-y-2 border-t border-slate-100 pt-4"><Label htmlFor={`occurrence-${visit.id}`}>Registro obrigatório da ocorrência / visita</Label><Textarea id={`occurrence-${visit.id}`} value={visit.occurrenceReport} onChange={(event) => onOccurrenceChange(event.target.value)} placeholder="Descreva o que foi observado, realizado ou encaminhado nesta visita." required minLength={8} /><p className="text-xs text-slate-500">{occurrenceReady ? "Relato pronto para envio." : "Informe pelo menos 8 caracteres antes de registrar a saída."}</p></div>}</CardContent></Card>;
}
