...
function shareText(report: any) {
  ...
  return [
    ...
    `Abastecimentos: ${metrics.fuelCount ?? 0} | Ocorrências registradas: ${metrics.occurrenceCount ?? 0}`,
  ].join("\n");
}
      ...
      <DialogContent className="max-h-[94vh] max-w-5xl overflow-y-auto border-0 bg-slate-50 p-0 text-slate-950 shadow-2xl">
        ...
        {isLoading || !report ? <div className="flex items-center justify-center gap-2 px-6 py-20 text-sm text-slate-500"><Loader2 className="h-5 w-5 animate-spin text-blue-700" /> Compilando relatório do turno...</div> : <div className="space-y-6 px-6 py-6 sm:px-8">
          <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4" aria-label="Indicadores do turno">
            ...
            <Kpi label="Coberturas / base" value={String(number(metrics.coverageCount))} detail={`${number(metrics.occurrenceCount)} ocorrência(s) registrada(s)`} icon={Route} tone="amber" />
            <Kpi label="Relatos pendentes" value={String(number(metrics.totalVisits) - number(metrics.occurrenceCount))} detail={`${number(metrics.pendingVisits)} visita(s) pendente(s)`} icon={attentionCount ? AlertTriangle : CheckCircle2} tone={attentionCount ? "rose" : "emerald"} />
          </section>
          ...
          <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
            ...
            <div className="mt-5 grid gap-4 lg:grid-cols-[1fr_auto] lg:items-center"><div><div className="flex items-center justify-between text-xs font-semibold text-slate-600"><span>Progresso das visitas</span><span>{completedVisits} de {totalVisits} concluída(s)</span></div><div className="mt-2 h-2.5 overflow-hidden rounded-full bg-slate-100"><div className="h-full rounded-full bg-blue-700 transition-all" style={{ width: `${visitProgress}%` }} /></div></div><div className="grid grid-cols-2 gap-x-6 gap-y-2 text-xs text-slate-600"><span><strong className="text-slate-950">{number(metrics.fuelCount)}</strong> abastecimento(s)</span><span><strong className="text-slate-950">{formatCurrency(metrics.fuelAmount)}</strong> em combustível</span><span><strong className="text-slate-950">{number(metrics.occurrenceCount)}</strong> ocorrência(s) registrada(s)</span><span><strong className="text-slate-950">{number(metrics.pendingVisits)}</strong> pendente(s)</span></div></div>
          </section>
